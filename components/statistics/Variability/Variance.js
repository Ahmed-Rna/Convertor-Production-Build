...
function Variance() {
  ...
  const calculateVariance = () => {
    ...
    Alert.alert('Variance Calculated', `The variance is: ${variance.toFixed(2)}`);
  };

  return (
    <View style={tw`flex-1 bg-gray-800 p-6`}>
      {/* Title */}
      <Text style={tw`text-white text-3xl font-bold mb-6 text-center`}>Variance Calculator</Text>
      ...
      {/* Submit Button */}
      <TouchableOpacity
        onPress={calculateVariance}
        style={tw`bg-purple-500 px-4 py-3 rounded-lg shadow-lg mt-2`}
      >
        <Text style={tw`text-white text-lg font-bold text-center`}>Calculate Variance</Text>
      </TouchableOpacity>

      {/* Footer */}
      <View style={tw`mt-6`}>
        <Text style={tw`text-gray-300 text-sm text-center`}>
          Enter a list of numbers separated by commas to calculate the variance.
        </Text>
      </View>
    </View>
  );
